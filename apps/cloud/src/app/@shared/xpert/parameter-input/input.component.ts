...
export class XpertParameterInputComponent {
  ...
  addAttr(item: TXpertParameterAttr) {
    this.value$.update(write((state) => {
      updateParameterByPath(state, item.path, (param) => {
        param.item ??= []
        param.item.push({type: XpertParameterTypeEnum.STRING} as TXpertParameter)
      })
      return state
    }))
  }
...
function expandAttributes(
  ...
  if (
    param.name && param.type === XpertParameterTypeEnum.OBJECT &&
    Array.isArray(param.item)
  ) {
    param.item.forEach(child =>
      attrs.push(...expandAttributes(child, currentPath, seen))
    )
  }

  if (
    param.name && param.type === XpertParameterTypeEnum.ARRAY &&
    Array.isArray(param.item)
  ) {
    param.item.forEach(child =>
      attrs.push(...expandAttributes(child, currentPath, seen))
...
function updateParameterByPath(
  ...
  if (
    (param.type === XpertParameterTypeEnum.OBJECT ||
      param.type === XpertParameterTypeEnum.ARRAY) &&
    Array.isArray(param.item)
  ) {
