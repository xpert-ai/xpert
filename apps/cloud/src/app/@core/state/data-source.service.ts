...
export type DataSourceAuthenticationInput = Pick<IDataSourceAuthentication, 'username' | 'password'> & {
  remeberMe?: boolean
}

export type DataSourcePingInput = Omit<Partial<IDataSource>, 'authentications'> & {
  authentications?: DataSourceAuthenticationInput[]
}
...
export class DataSourceService extends OrganizationBaseService {
  ...
  getAll(relations: string[] = []) {
    ...
    return this.selectOrganizationId().pipe(
      switchMap(() => {
        return this.#http
          .get<{ items: IDataSource[]; totals: number }>(C_API_DATA_SOURCE, {
            params
          })
  ...
  getOne(id: string) {
    return this.#http.get<IDataSource>(C_API_DATA_SOURCE + `/${id}`, {
      params: {
  ...
  ping(dataSource: DataSourcePingInput): Observable<unknown>
  ping(id: string, dataSource: DataSourcePingInput): Observable<unknown>
  ping(id: string | DataSourcePingInput, dataSource?: DataSourcePingInput): Observable<unknown> {
    if (typeof id === 'string') {
  ...
  createAuthentication(id: string, auth: DataSourceAuthenticationInput) {
