...
export class CrudService<T> {
  ...
  getAll(options?: PaginationParams<T>) {
    return this.httpClient.get<{ items: T[]; total: number }>(this.apiBaseUrl, { params: toHttpParams(options) })
  }

  getMyAll(options?: PaginationParams<T>) {
    return this.httpClient.get<{ items: T[]; total: number }>(this.apiBaseUrl + '/my', {
