...
export class InviteService {
  ...
  validateInvite(relations: string[], findInput: IPublicInviteFindInput): Promise<IInvite> {
    const data = JSON.stringify({ relations, findInput })
  ...
  acceptEmployeeInvite(acceptInviteInput: IInviteAcceptInput): Promise<any> {
    return firstValueFrom(this.http.post(`${API_PREFIX}/invite/employee`, acceptInviteInput))
  }
  acceptCandidateInvite(acceptInviteInput: IInviteAcceptInput): Promise<any> {
    return firstValueFrom(this.http.post(`${API_PREFIX}/invite/candidate`, acceptInviteInput))
  }
  acceptUserInvite(acceptInviteInput: IInviteAcceptInput): Promise<any> {
    return firstValueFrom(this.http.post(`${API_PREFIX}/invite/user`, acceptInviteInput))
  }
  ...
  inviteOrganizationContact(organizationContactId: string): Promise<IOrganizationContact> {
    return firstValueFrom(
      this.http.put<IOrganizationContact>(`${API_PREFIX}/invite/organization-contact/${organizationContactId}`, {})
    )
  }
