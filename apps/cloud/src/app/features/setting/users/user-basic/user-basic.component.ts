...
export class UserBasicComponent {
  private readonly userComponent = inject(XpEditUserComponent)
  ...
  async save() {
    ...
    try {
      ...
      this.toastrService.success(`XP.NOTES.USERS.USER_UPDATED`, { name: new CreatedByPipe().transform(this.user) })
