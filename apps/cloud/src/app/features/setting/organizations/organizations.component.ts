...
import { CURRENT_USER_FULL_RELATIONS, UsersService } from '@cloud/app/@core/state'
...
type OrganizationDetailsTab = 'general' | 'members' | 'user-groups' | 'controls' | 'tags' | 'demo' | 'smtp'
  ...
  constructor() {
    ...
    effect(
      () => {
        if (
          (this.isTenantScope() &&
            (this.selectedTab() === 'tags' || this.selectedTab() === 'demo' || this.selectedTab() === 'smtp')) ||
          (this.selectedTab() === 'smtp' && !this.canViewCustomSmtp())
        ) {
