...
@Component({
  standalone: true,
  selector: 'xp-settings-assistants-organization-page',
          ...
          <div class="space-y-6">
            <section class="rounded-3xl border border-divider-regular bg-components-card-bg px-5 py-5">
              <div class="text-xs uppercase tracking-[0.24em] text-text-tertiary">
                {{ 'XP.Assistant.OrganizationScope' | translate: { Default: 'Organization Scope' } }}
              </div>
              <div class="mt-2 text-2xl font-semibold text-text-primary">
                {{
                  facade.organizationName() ||
                    ('XP.Assistant.OrganizationOverride' | translate: { Default: 'Organization Override' })
                }}
              </div>
              <p class="mt-2 max-w-3xl text-sm text-text-secondary">
                {{
                  'XP.Assistant.OrganizationPageDesc'
                    | translate
                      : {
                          Default:
            ...
            <xp-settings-assistants-scope [scope]="assistantBindingScope.ORGANIZATION" />
          </div>
        }
      </div>
    </div>
