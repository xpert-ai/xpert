...
describe('chat routes', () => {
  ...
  it('routes /chat/clawxpert to the ClawXpert page', () => {
    ...
    expect(route?.children?.find((item) => item.path === '')?.redirectTo).toBe('c')
    expect(route?.children?.find((item) => item.path === 'settings')?.redirectTo).toBe('c')
    expect(route?.children?.some((item) => item.component === ClawXpertOverviewComponent)).toBe(false)
