...
export class XpertStudioContextMenuComponent {
  ...
  readonly imageUnderstandings$ = this.knowledgebaseAPI.understandingStrategies$.pipe(
    map((items) => items.map((_) => _.meta))
  )
  ...
  async addWorkflowRouter(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.IF_ELSE).length ?? 0
  ...
  addWorkflowIterator(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.ITERATOR).length ?? 0
  ...
  addWorkflowAnswer(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.ANSWER).length ?? 0
  ...
  addWorkflowQuestionClassifier(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.CLASSIFIER).length ??
      0
  ...
  addWorkflowKnowledgeRetrieval(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.KNOWLEDGE).length ??
      0
  ...
  addWorkflowCode(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.CODE).length ?? 0
  ...
  addWorkflowTemplate(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.TEMPLATE).length ?? 0
  ...
  addWorkflowJSONStringify(fromNode: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.JSON_STRINGIFY)
        .length ?? 0
  ...
  addWorkflowJSONParse(fromNode: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.JSON_PARSE).length ??
      0
  ...
  addWorkflowVariableAssigner(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.ASSIGNER).length ?? 0
  ...
  async addWorkflowHttp(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.HTTP).length ?? 0
  ...
  async addWorkflowTool(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.TOOL).length ?? 0
  ...
  async addWorkflowSubflow(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.SUBFLOW).length ?? 0
  ...
  addWorkflowListOperator(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.LIST_OPERATOR)
        .length ?? 0
  ...
  addWorkflowVariableAggregator(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.VARIABLE_AGGREGATOR)
        .length ?? 0
  ...
  addWorkflowTask(fromNode?: TXpertTeamNode) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.TASK).length ?? 0
    ...
    const hasTrigger =
      ...
      this.nodes()?.some(
        (n) =>
          ...
          (<IWFNTrigger>n.entity).from === 'chat'
      )
    if (hasTrigger) {
      this.#toastr.danger(
  ...
  addPipelineSource(provider: IDocumentSourceProvider) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.SOURCE).length ?? 0
  ...
  addPipelineProcessor(provider: IDocumentProcessorProvider) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.PROCESSOR).length ??
      0
  ...
  addPipelineChunker(provider: IDocumentChunkerProvider) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.CHUNKER).length ?? 0
  ...
  addPipelineUnderstanding(provider: IDocumentUnderstandingProvider) {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.UNDERSTANDING)
        .length ?? 0
  ...
  addSkill() {
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.SKILL).length ?? 0
  ...
  addMiddleware(provider: string) {
    if (!provider) return
    const length =
      this.nodes()?.filter((n) => n.type === 'workflow' && n.entity?.type === WorkflowNodeTypeEnum.MIDDLEWARE).length ??
      0
