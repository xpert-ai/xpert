...
import { NbAuthJWTToken, NbAuthSimpleToken, xpAuthCreateToken } from './token'
import { XP_AUTH_FALLBACK_TOKEN, XpAuthTokenParceler } from './token-parceler'

describe('token-storage', () => {
  let tokenStorage: XpTokenStorage
  let tokenParceler: XpAuthTokenParceler
  ...
  beforeEach(waitForAsync(
    inject([XpTokenStorage, XpAuthTokenParceler], (_tokenStorage, _tokenParceler) => {
  ...
  it('set test token', () => {
    const token = xpAuthCreateToken(NbAuthSimpleToken, testTokenValue, ownerStrategyName)
  ...
  it('setter set invalid token to localStorage as empty string', () => {
    ...
    token = xpAuthCreateToken(NbAuthSimpleToken, null, ownerStrategyName)
    tokenStorage.set(token)
    expect(localStorage.getItem(testTokenKey)).toEqual(tokenParceler.wrap(token))

    token = xpAuthCreateToken(NbAuthSimpleToken, undefined, ownerStrategyName)
    tokenStorage.set(token)
    expect(localStorage.getItem(testTokenKey)).toEqual(tokenParceler.wrap(token))
  })
  ...
  it('should return correct value', () => {
    const token = xpAuthCreateToken(NbAuthSimpleToken, 'test', ownerStrategyName)
