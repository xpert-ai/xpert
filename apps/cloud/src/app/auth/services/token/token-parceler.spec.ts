...
import { NbAuthSimpleToken, NbAuthJWTToken, xpAuthCreateToken } from './token'
...
describe('token-parceler', () => {
  ...
  const simpleToken = xpAuthCreateToken(NbAuthSimpleToken, 'test value', 'strategy', createdAt)
  ...
  // tslint:disable-next-line
  const jwtToken = xpAuthCreateToken(
    NbAuthJWTToken,
  ...
  describe('default configuration', () => {
    ...
    beforeEach(waitForAsync(
      inject([XpAuthTokenParceler], (_tokenParceler) => {
  ...
  describe('fail configuration', () => {
    beforeEach(() => {
      TestBed.configureTestingModule({
        providers: [
          { provide: XP_AUTH_FALLBACK_TOKEN, useValue: NbAuthSimpleToken },
          { provide: XP_AUTH_TOKENS, useValue: [] },
          XpAuthTokenParceler
        ]
      })
    })
