import type { IPluginConfig } from '@xpert-ai/server-common'
...
export async function runTenantAdminCli(
  ...
  const seedTenant =
    dependencies.seedTenant ?? ((tenantName: string) => seedModule(pluginConfig, { name: 'Tenant', tenantName }))
...
async function seedModule(pluginConfig: Partial<IPluginConfig>, options: { name: string; tenantName: string }) {
  const server = await import('@xpert-ai/server-core')

  return server.seedModule(pluginConfig, options)
}
...
async function createTenantAdminApp(pluginConfig: Partial<IPluginConfig>) {
...
async function updateTenant(args: TenantAdminArgs, service: TenantAdminService, output: (value: unknown) => void) {
  ...
  const hasName =
    typeof args.name === 'string' || typeof args.tenantName === 'string' || typeof args.tenant === 'string'
