...
function isNullishType(typeNode) {
  return nullishKinds.has(typeNode.kind) || (ts.isLiteralTypeNode(typeNode) && nullishKinds.has(typeNode.literal.kind))
}
...
function classifyUnsafeColumnType(typeNode) {
  ...
  if (isLiteralUnion(typeNode) || ts.isUnionTypeNode(typeNode)) {
    return {
      ...
      advice:
        "add an explicit TypeORM column type, usually type: 'varchar' for string unions or type: 'int' for numeric unions"
    }
  }
