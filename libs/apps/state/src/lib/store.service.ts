...
export type PermissionKey = IRolePermission['permission']
...
export function createInitialPersistState(): PersistState {
  ...
  const activeScopeLevel =
    (localStorage.getItem('_activeScopeLevel') as RequestScopeLevel) || RequestScopeLevel.ORGANIZATION
  const lastOrganizationId = localStorage.getItem('_lastOrganizationId') || organizationId || null
  const lastTenantCompatibleRoute = localStorage.getItem('_lastTenantCompatibleRoute') || null
  const lastOrganizationCompatibleRoute = localStorage.getItem('_lastOrganizationCompatibleRoute') || null
...
export class Store {
  ...
  preferredTheme$ = this.persistQuery.select((state) => state.preferredTheme)
  ...
  get organizationId(): IOrganization['id'] | null {
    return this.activeScope.level === RequestScopeLevel.ORGANIZATION ? this.activeScope.organizationId : null
  }
  ...
  set organizationId(id: IOrganization['id'] | null) {
    this.persistStore.update((state) => ({
      ...state,
      organizationId: id,
      activeScopeLevel: id ? RequestScopeLevel.ORGANIZATION : RequestScopeLevel.TENANT,
  ...
  loadPermissions() {
    ...
    let permissions = []
    const { userRolePermissions } = this.appQuery.getValue()
    const userPermissions = userRolePermissions
    ...
    if (this.isOrganizationScope && selectedOrganization) {
      const organizationPermissions = Object.keys(OrganizationPermissionsEnum)
  ...
  selectOrganizationId() {
    return this.activeScope$.pipe(
      map((scope) => (scope.level === RequestScopeLevel.ORGANIZATION ? scope.organizationId : null)),
      distinctUntilChanged()
    )
  }
  ...
  restoreRememberedScope(userId: string | null | undefined) {
    ...
    this.persistStore.update((state) => ({
      ...
      organizationId:
        rememberedScope.level === RequestScopeLevel.ORGANIZATION ? (rememberedScope.organizationId ?? null) : null,
      lastOrganizationId: rememberedScope.organizationId ?? state.lastOrganizationId ?? null
    }))
  }
  ...
  setLastCompatibleRoute(level: RequestScopeLevel, route: string | null) {
    this.persistStore.update((state) => ({
      ...state,
      lastTenantCompatibleRoute: level === RequestScopeLevel.TENANT ? route : state.lastTenantCompatibleRoute,
      lastOrganizationCompatibleRoute:
        level === RequestScopeLevel.ORGANIZATION ? route : state.lastOrganizationCompatibleRoute
    }))
  }

  getLastCompatibleRoute(level: RequestScopeLevel) {
    const state = this.persistQuery.getValue()
    return level === RequestScopeLevel.TENANT ? state.lastTenantCompatibleRoute : state.lastOrganizationCompatibleRoute
  }
...
function resolveActiveScopeFromPersistState(state: PersistState): ActiveScope {
  const level = state.activeScopeLevel ?? RequestScopeLevel.ORGANIZATION
...
function isActiveScopeEqual(a: ActiveScope, b: ActiveScope) {
  return (
    ...
    (a?.level !== RequestScopeLevel.ORGANIZATION ||
      a.organizationId === (b.level === RequestScopeLevel.ORGANIZATION ? b.organizationId : null))
  )
}
