...
import { distinctUntilChanged, filter, startWith, withLatestFrom } from 'rxjs/operators'

@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  selector: 'xp-formly-json',
  standalone: false,
  templateUrl: './json.type.html',
  styleUrls: ['./json.type.scss']
})
export class XpFormlyJsonComponent extends FieldType implements OnInit {
  @HostBinding('class.xp-formly-json')
  ...
  ngOnInit() {
    ...
    this.fControl.valueChanges.pipe(distinctUntilChanged()).subscribe((value) => {
      try {
