...
- The default working directory is the current user's private operation area for the current context.
- Shared directories must be used explicitly; the default cwd is not a shared area.
...
| ---------------------------------------- | ------------------------------- |
| Default cwd                              | `users/<userId>`                |
| Project shared workspace                 | `shared`                        |
...
| -------------------------- | ------------------------------- |
| Default cwd                | `users/<userId>`                |
| Xpert shared file area     | `shared`                        |
| Xpert long-term FileMemory | `.xpert/memory`                 |

So when multiple users talk to the same Xpert:

- File operations default to each user's own `xpert/<xpertId>/users/<userId>`.
- Shared work must explicitly use `xpert/<xpertId>/shared`.
...
| Directory                                       | Purpose                                                        |
| ----------------------------------------------- | -------------------------------------------------------------- |
| `/sandbox/tenant-1/xpert/xpert-X/users/user-A`  | Default file-operation directory for user A talking to Xpert X |
| `/sandbox/tenant-1/xpert/xpert-X/shared`        | Shared file area for Xpert X; must be used explicitly          |
...
| ----------------------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------- |
| `/sandbox/tenant-1/xpert/xpert-X/users/user-A`  | `/workspace/users/user-A`  | Default cwd; ordinary generated files, uploads, and terminal commands write here by default |
| `/sandbox/tenant-1/xpert/xpert-X/shared`        | `/workspace/shared`        | Explicit shared directory                                                                   |
...
If user B talks to the same Xpert X, the default file directory becomes:
