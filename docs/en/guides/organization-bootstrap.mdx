...
flowchart TD
  ...
  H --> V["Initialize semantic model data"]
...
- idempotently register default skill repositories from a fixed YAML file
...
Organization bootstrap also seeds skill repositories from the repo-local file:
...
- repositories removed from YAML are not deleted from existing organizations
