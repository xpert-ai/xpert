...
`PluginInstance` 继续表示某个 plugin 已安装到组织/系统作用域，并承载插件级必要状态，例如 package、version、source、sourceConfig、config、configurationStatus。当前资源能力不需要新增独立组件实例表；如果后续需要记录 connector 覆盖、组织默认策略等插件级运行态，优先放入 `PluginInstance.config`，并通过明确的 parser 读取。
...
- 必要的 connector 覆盖信息放在 `PluginInstance.config` 中，通过显式边界 parser 读取，不新增表。
