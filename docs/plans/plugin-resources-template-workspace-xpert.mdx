...
- `resources/state` 查询 workspace target 时会用 `sharedSkillId` 反查已有 plugin skill package；即使安装记录缺失，也能把既有 skill 识别为已安装，避免重复显示为可安装。
...
- template/workspace install 需要 workspace write 权限。
- existing Xpert install 根据 Xpert 所属 workspace 校验 write 权限。
