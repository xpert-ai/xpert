...
状态：第一、二批已合并；第三批 AnyDoc、第四批 OpenDataLoader 及宿主接入已实现，正在按仓库提交。插件发布、生产镜像发布和部署验收仍需独立完成。

### 实施进度（截至 2026-09-16 提交准备）
...
| 第三批：AnyDoc                                    | 插件、Node Runtime、导入适配及扫描页图像理解已实现                                                                                   | 转换在受管环境执行；扫描页由所选视觉模型转录 |
| 第四批：OpenDataLoader PDF                        | PDF 插件、Java Runtime、本地 Hybrid OCR 和置信度配置已实现                                                                           | OCR 依赖与模型由 Runtime 预装；仅声明 PDF    |
...
状态：已实现 `xpertai/documents/anydoc/`，Action 为 `1.0.3`。本地转换、适配、打包和生命周期已有验证；生产目标镜像与完整知识库矩阵仍需验收。

### 6.1 本批要交付什么

新增 `@xpert-ai/plugin-anydoc`，作为知识库“解析引擎”中的一个本地文档转换选项，重点补充旧版 Office、RTF、EPUB 和 OpenDocument 格式。

推荐实现：**官方 Node.js 绑定 + 平台 Sandbox Jobs + 预装依赖的文档 Node Runtime**。解析结果接回已有资产存储、图像理解、分块和索引流程。
...
先按下表准备真实样例和转换测试，再将通过验证的格式加入 `supportedFileTypes`。不能只因为上游 README 列出就全部宣称可用。
...
新增平台 Runtime profile，暂定 **`document/node-20/v1`**，镜像 family 为 `document-node`。它使用项目已有 Node 20 基础镜像与 Runner Host，预装锁定的 anydoc 及对应平台原生包。
...
当前 Runner Host 只为 Playwright 显式暴露依赖，不能假设 Action 中普通 `import` 自动找到新镜像中的 anydoc。计划由 Runtime 提供可信依赖根目录，Action 使用 `createRequire` 从该目录解析 `@firecrawl/anydoc`。本地 Provider 和镜像保持相同约定，路径不能来自上传文档或解析参数。
...
提交按仓库拆分：宿主目标 `develop`，分为 Runtime、知识库接入、图像理解、通用 Slider 和文档五个提交；插件目标 `main`，分为 AnyDoc、OpenDataLoader 和 rag-vlm 修复三个提交。Changeset 均使用 patch。
...
2. 图片与嵌入对象的实际 MIME 分布，以及现有资产／VLM 链路对它们的处理；PDF 无资产模型的限制保持可见。
...
状态：已实现 `xpertai/documents/opendataloader/`，Action 为 `1.2.1`。受管 Java 环境、Hybrid OCR、依赖健康检查、可配置置信度及转换测试已接通；生产镜像发布与部署验收独立完成。
...
本次查到最新正式发行版为 **OpenDataLoader PDF `v2.5.8`**，发布于 2026-09-11，作为本批候选锁定版本。官方 CLI 发行物是 ZIP 包，构建镜像时下载、校验摘要、解包并固定 JAR 路径；运行任务时不下载、不执行 Maven 构建。[官方发行版](https://github.com/opendataloader-project/opendataloader-pdf/releases/tag/v2.5.8)
...
资源初始建议与文档 Runtime 保持同量级：容器内存 4 GiB、JVM 最大堆 2 GiB、输入 100 MiB、总输出 128 MiB、执行 300 秒、硬截止 360 秒。给 JVM 的堆之外保留原生内存、字体、图片和 Node 开销；最终值由冷启动、大页数和图片 PDF 样例验证。
...
- `providesImageText: false`。图片提取不等于 OCR，原生 Java 解析不宣称完成扫描页识别。
