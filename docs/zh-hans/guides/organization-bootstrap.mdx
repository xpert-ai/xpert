...
flowchart TD
  ...
  I --> J["Register default skill repositories from YAML"]
...
- 从仓库内固定 YAML 幂等注册默认 skill repositories
...
组织 bootstrap 还会从仓库内的固定文件补齐默认 skill repositories：

```text
packages/server-ai/src/initialization/default-skill-repositories.yaml
```
...
- 若已存在则执行更新，不会重复创建
- YAML 中后续删除的条目不会反向删除组织里已存在的自定义 repository
