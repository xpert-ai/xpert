...
Xpert 把“任务编排”“运行时定义”“运行时实现”和“插件功能”拆成独立层：OSS Core 维护通用 Job 语义和 Provider SPI，Runtime Suite 发布提供方无关的 Definition 与 OCI artifacts，system-level 插件维护可审核的 Action Bundle，Pro 或其他发行版通过 Binding 接入具体 Provider。
...
flowchart LR
    ...
    Q --> W["Provider 发行物 Worker<br/>Pro: sandbox-runtime-worker"]
...
Provider 发行物的 Worker 不是第二套 Job 编排系统。它只是专门消费 `sandbox-browser` execution pool 的进程形态；幂等、容量、容器生命周期、输入输出和清理由 `Sandbox Jobs Runtime` 完成。API 在开发和生产环境都不消费浏览器池、不初始化 Runtime Provider。OSS 保留通用 `bootstrapWorker()` 扩展入口，但有意不部署该进程；具体 Provider 发行物负责提供 Worker 服务。

## 平台 Runtime Suite package

`packages/sandbox-runtime` 是私有 package `@xpert-ai/sandbox-runtime`，只集中维护 OCI image 资产，不包含 Provider 或业务编排：
...
OSS Core 从 `@xpert-ai/sandbox-runtime` 构建产物注册通用的 `browser/playwright-1.61/v1`：
...
Definition 不包含 provider、image、宿主路径或插件标识。它在 OSS 与 Pro 中完全相同，不要求用户配置 profile 文件或环境变量。
...
Registry 会拒绝绝对路径、路径穿越、空字节、软链接、硬链接和非普通文件；默认限制为 256 MiB 和 20,000 个文件。它逐文件校验 size/SHA-256，再校验确定性 tree hash。验证后的不可变内容按 `bundleSha256` 在 worker 内缓存，随后复制到每个 Job 的 `/workspace/runtime/action`。插件安装目录、宿主路径和插件 `node_modules` 从不挂载到 Sandbox。
...
Health 依次检查 Action 是否存在、bundle 是否有效、Runtime contract/Playwright 是否兼容、Definition 是否存在、Worker heartbeat、Binding、不可变 artifact、Provider 和实际 Runtime manifest。`RUNTIME_UNBOUND` 表示 Worker 在线但没有兼容 Binding；没有有效 Worker heartbeat 则是 `PROVIDER_UNAVAILABLE`。
...
| `SandboxRuntimeDefinitionRegistry`   | 从 Runtime Suite 加载提供方无关的 Runtime Definition。                                                             |
| `SandboxRuntimeBindingSelector`      | 过滤必需能力与不可变 artifact，执行 health 检查，再按优先级确定性选择 Binding。                                    |
| `SandboxRuntimeHealthService`        | Worker 探测并发布有 TTL 的 Runtime health；API 只聚合 Redis heartbeat。                                            |
| `SandboxJobRuntimeCapabilityService` | 负责幂等、容量、materialize、执行、输出校验、运行证据、取消和清理；只有 worker 执行路径可以创建 Runtime instance。 |
| `SandboxJobCapacityService`          | 在创建 Runtime 前原子获取全局/tenant/user 有期限租约，并幂等释放。                                                 |
| `bootstrapWorker()`                  | 在 `@xpert-ai/xpert-api` 组装无 HTTP worker；OSS 不部署该服务，具体 Provider 发行物负责部署。                      |
...
通用 Job 生命周期位于 OSS `server-ai`；具体容器生命周期由 Provider SPI 实现，不进入 `packages/sandbox-runtime`。Docker 实现暂存在私有 system-level 插件 `xpertai/providers/sandbox-docker`，未来可整体移入 Pro 仓库。
...
3. 除 `playwright-core` 外的 JS 依赖进入 bundle；Playwright 由 Browser Runtime 提供。
4. 构建生成 `action.json` 和确定性 tree hash。
5. 插件 manifest 的 `sandboxActions` 指向该 Action manifest。
...
HTML 继续走无浏览器路径。生产强制 `exportBackend='sandbox-job'`；`local` 与 `chromiumExecutablePath` 只保留给 development/test。PDF/PPTX 默认开启，不再提供 `sandboxExportEnabled` 开关；Action、Definition、Binding、Provider artifact 或 worker 条件不足时，通过 capability warning 明确提示并保持 HTML 可用。
...
Worker 启动后立即探测所有 Runtime Definition，随后每 15 秒写入一次 Redis heartbeat，TTL 为 45 秒。API 只读 heartbeat，不连接 Docker socket；真正执行前 Worker 会再次本地选择和验证 Binding。

Worker 的无 HTTP `bootstrapWorker()` 由 `@xpert-ai/xpert-api` 入口拥有并负责组合插件、队列和领域模块。Analytics 只是被加载的领域模块，不负责选择进程角色或启动 Worker。

OSS Compose 不部署 Sandbox Runtime Worker、不挂载 Docker socket，也不拉取 Runtime artifact；它仍提供 `bootstrapWorker()`，使社区 Provider 发行物无需 fork API bootstrap 即可组装 Worker。没有有效 heartbeat 时 health 返回 `PROVIDER_UNAVAILABLE`；Action/Runtime 健康但浏览器执行池没有消费者时返回 `WORKER_UNAVAILABLE`。Presentation Studio 在这两种情况下都不会入队 PDF/PPTX 任务，而是显示可操作 warning，并保持 HTML 可用。

私有 Docker Provider 拥有 Pro 部署 overlay。它默认启动 `sandbox-runtime-worker` 服务，当前内部进程角色如下；队列选择、并发、容量、Provider 注册和不可变 Runtime artifact 都由发行物默认值提供：

| 变量                                        | 用途                         |
| ------------------------------------------- | ---------------------------- |
| `XPERT_PROCESS_ROLE=sandbox-browser-worker` | 启动无 HTTP worker bootstrap |

Pro overlay 默认两个 Worker，单实例默认并发 5，容量默认全局 20、单租户 4、单用户 2。它使用 API 镜像以加载插件 handler 和平台 Runtime，但镜像本身不含 Chromium；Chromium 只存在于短生命周期 Browser Runtime 容器。只有 Pro Worker 挂载 Docker socket；API 和所有 OSS 基础服务都不挂载。Provider 自带由 CI 生成的 `runtime-suite.lock.json` 并选择不可变 digest；Pro 用户无需设置 Worker 开关、image、profile、provider 或 `CHROME_PATH`。Provider 开发环境自动使用 `xpert-sandbox-browser:local`，缺失时 health warning 会给出构建提示。

开源版默认提供 Core、SPI、Definition、Action、心跳契约和测试 Fake Provider，但不内置生产 Provider 或 Worker 服务。社区可以通过公开 SPI 实现 Podman、Kubernetes 或 Remote Provider，并随 Provider 自行打包 Worker 部署。`RUNTIME_UNBOUND` 只表示 Worker 在线但无法绑定兼容 Runtime Definition。Docker 实现及其部署 overlays 目前暂存在私有 system-level 插件 `xpertai/providers/sandbox-docker`，后续可整体迁往 Pro 仓库。
...
Pro Provider release CI 从 Runtime Artifact Catalog 生成自己的不可变 `runtime-suite.lock.json`，生产 lock 缺失或含 tag 时直接失败。推荐发布顺序：平台 Contracts/SDK/Core → Runtime Suite → Provider lock 与 worker → 插件 Action Bundle。回滚优先恢复上一 Runtime artifact digest；Runtime 故障时平台提示 PDF/PPTX 不可用，并保持 HTML 可用。
