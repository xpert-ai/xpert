...
export class DataSourceService extends TenantOrganizationAwareCrudService<DataSource> {
	...
	async ping(id: string | IDataSource, dataSource?: IDataSource) {
		...
		const runner: DBQueryRunner = await this.queryBus.execute(
			new DataSourceStrategyQuery(
				...
				(_dataSource.options ?? {}) as unknown as AdapterBaseOptions
			)
		)
	...
	async findSqlDataSources() {
		const [items, total] = await this.repository
			...
			.where(
				'dataSource.tenantId = :tenantId and dataSource.organizationId = :organizationId and type.protocol = :protocol',
				{
