...
import { ViewHostDefinitionContract } from '../host-definition.interface'
...
export class IntegrationViewHostDefinition implements ViewHostDefinitionContract {
	...
	async resolve(hostId: string) {
		...
		return {
			hostSnapshot: {
				...
				workspace:
					getStringProperty(integration.options, 'teamName') ??
					getStringProperty(integration.options, 'workspace'),
				team:
					getStringProperty(integration.options, 'teamName') ??
					getStringProperty(integration.options, 'team'),
				botUser:
					getStringProperty(integration.options, 'botUserName') ??
					getStringProperty(integration.options, 'botUserId'),
	...
	canRead() {
		return RequestContext.hasAnyPermission(
			[PermissionsEnum.INTEGRATION_VIEW, PermissionsEnum.INTEGRATION_EDIT],
			false
		)
	}
}
