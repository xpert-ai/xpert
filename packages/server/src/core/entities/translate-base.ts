...
export abstract class TranslationBase extends TenantOrganizationBaseEntity implements ITranslation<TranslatableBase> {
	reference: ITranslatable<TranslatableBase>
	languageCode: string
}

export abstract class TranslatableBase extends TenantOrganizationBaseEntity implements ITranslatable<TranslationBase> {
	...
	translateNested(languageCode: string, translatePropsInput: Array<TranslatePropertyInput>): any {
		...
		Object.keys(this).forEach((prop: string) => {
			...
			currentInputProp = inputProps.find((el: TranslatePropertyInput) => el.propAsArr[0] == prop)

			if (!currentInputProp) {
				result[prop] = element[prop]
			} else {
				let inputKeys = inputProps.find((el: TranslatePropertyInput) => el.propAsArr[0] == prop).propAsArr

				if (prop == 'root') {
					elementPropTranslations = element ? element.translations : []
				} else if (inputKeys.length == 1) {
					elementPropTranslations = element[inputKeys[0]] ? element[inputKeys[0]].translations : []
				...
				let elementPropTranslation =
					elementPropTranslations.find(
						(translation: ITranslation<TranslationBase>) => translation.languageCode == languageCode
					) || null
				...
				if (elementPropTranslations && elementPropTranslation) {
					...
					propsTranslateKeys.forEach((translateKeyInput) => {
						result[translateKeyInput.alias] = elementPropTranslation[translateKeyInput.key] || ''
					})
				}
			}
		})
