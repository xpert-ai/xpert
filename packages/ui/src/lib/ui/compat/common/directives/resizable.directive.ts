...
export class NgmResizableDirective implements OnDestroy {
  ...
  private createHandle(direction: ResizeDirection): void {
    ...
    if (['left', 'right'].includes(direction)) {
      ...
      this.renderer.listen(handle, 'mouseenter', () => this.renderer.setStyle(handle, 'background', 'rgba(0,0,0,0.15)'))
      this.renderer.listen(handle, 'mouseleave', () => this.renderer.setStyle(handle, 'background', 'rgba(0,0,0,0.05)'))
