...
import { NgmCopyComponent } from '../common/copy/copy.component'
...
@Component({
  ...
  template: `<pre class="m-0 overflow-auto"><code [innerHTML]="highlightedCode()" class="m-0"></code></pre>
    <ngm-copy
      class="absolute right-1 top-1 text-lg !text-primary-foreground rounded-md w-6 h-6 flex justify-center items-center bg-primary/30 backdrop-blur-sm"
      [content]="code()"
    /> `,
  styles: [
    `
