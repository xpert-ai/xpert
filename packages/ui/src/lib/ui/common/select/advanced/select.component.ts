...
import { ZardCheckboxComponent } from '../../../../components/checkbox'
import {
  ZardComboboxDeprecatedComponent,
  ZardComboboxDeprecatedPanelTemplateDirective,
  type ZardComboboxDeprecatedOption
} from '../../../../components/combobox-deprecated'
import { ZardFormImports } from '../../../../components/form'
import { ZardIconComponent } from '../../../../components/icon'
import { ZardInputDirective } from '../../../../components/input'
import { ZardLoaderComponent } from '../../../../components/loader'
import { DisplayDensity, DensityDirective, ISelectOption, NgmFieldAppearance, NgmFieldColor } from '../../core'
import { DisplayBehaviour } from '../../core'
...
export class NgmAdvancedSelectComponent implements OnChanges, ControlValueAccessor {
  ...
  readonly isNotInitial = computed(() => (this.multiple ? this.selectedValues().length : !!this.singleValue()))
  ...
  readonly filteredComboboxOptions = computed(() => {
    ...
    return this.comboboxOptions().filter((option) => {
      ...
      return original?.caption?.toLowerCase().includes(text) || `${original?.key ?? ''}`.toLowerCase().includes(text)
    })
  })
