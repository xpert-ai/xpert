...
const resizeHandleBackgroundClass = 'bg-muted-foreground/20'
const resizeHandleHoverBackgroundClass = 'bg-muted-foreground/40'
const resizeCornerBackgroundClass = 'bg-muted-foreground/40'
const resizeCornerHoverBackgroundClass = 'bg-muted-foreground/70'
...
export class NgmResizableDirective implements OnDestroy {
  ...
  private createHandle(direction: ResizeDirection): void {
    ...
    this.renderer.addClass(handle, 'transition-colors')
    ...
    if (['left', 'right'].includes(direction)) {
      ...
      this.renderer.addClass(handle, resizeHandleBackgroundClass)
      this.renderer.listen(handle, 'mouseenter', () =>
        this.replaceClass(handle, resizeHandleBackgroundClass, resizeHandleHoverBackgroundClass)
      )
      this.renderer.listen(handle, 'mouseleave', () =>
        this.replaceClass(handle, resizeHandleHoverBackgroundClass, resizeHandleBackgroundClass)
      )
      ...
      const applyBarStyle = (bar: HTMLElement) => {
        this.renderer.addClass(bar, resizeCornerBackgroundClass)
        this.renderer.addClass(bar, 'transition-colors')
      ...
      this.renderer.listen(handle, 'mouseenter', () => {
        this.replaceClass(bar1, resizeCornerBackgroundClass, resizeCornerHoverBackgroundClass)
        this.replaceClass(bar2, resizeCornerBackgroundClass, resizeCornerHoverBackgroundClass)
      })
      this.renderer.listen(handle, 'mouseleave', () => {
        this.replaceClass(bar1, resizeCornerHoverBackgroundClass, resizeCornerBackgroundClass)
        this.replaceClass(bar2, resizeCornerHoverBackgroundClass, resizeCornerBackgroundClass)
      })
    }
  ...
  private replaceClass(element: HTMLElement, currentClass: string, nextClass: string) {
