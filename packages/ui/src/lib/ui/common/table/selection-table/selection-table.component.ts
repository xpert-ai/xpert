...
import { ZardTableImports, type ZardTableSortDirection } from '../../../../components/table'
import { ZardCheckboxComponent } from '../../../../components/checkbox'
import { type ZardPageEvent, ZardPaginatorComponent } from '../../../../components/pagination'
import { DensityDirective, DisplayDensity } from '../../core'
...
export class NgmSelectionTableComponent implements OnInit, ControlValueAccessor {
  ...
  readonly filteredRows = computed(() =>
    filterTableRowsByColumns(
      this.data() ?? [],
      this.columns().map(({ value }) => value),
      this.searchText()
    )
  )
  readonly rows = computed(() =>
    paginateTableRows(sortTableRows(this.filteredRows(), this.sortState()), this.pageIndex(), this.pageSize())
  )
