...
import { comboboxVariants, type ZardComboboxWidthVariants } from '@/src/lib/components/combobox/combobox.variants';
import {
...
} from '@/src/lib/components/command';
import { ZardEmptyComponent } from '@/src/lib/components/empty';
...
@Component({
  ...
  template: `
    ...
    <ng-template #commandOption let-option let-cmd="commandInstance" let-grp="groupInstance">
      ...
      >
        @if (optionTemplate(); as optionTemplateRef) {
          <ng-container
          ...
          @if (option.value === currentValue()) {
            <ng-icon name="lucideCheck" class="ml-auto shrink-0" />
          }
        } @else {
          @if (option.icon) {
            <ng-icon [name]="option.icon" class="mr-2 shrink-0 opacity-70" />
          }
