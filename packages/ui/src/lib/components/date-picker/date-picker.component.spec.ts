...
describe('ZardDatePicker family', () => {
  it('localizes the default day picker placeholder and allows overrides', async () => {
    const language = signal('zh-CN')
    ...
    expect(button.textContent).toContain('选择日期')
    ...
    fixture.componentRef.setInput('placeholder', 'Custom date')
    fixture.detectChanges()

    expect(button.textContent).toContain('Custom date')
  })
