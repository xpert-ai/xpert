...
export class ZardPaginatorComponent implements OnInit, OnDestroy, ZardPaginatorLike {
  ...
  protected readonly pageSizeClasses = computed(() =>
    mergeClasses('flex items-center gap-2', this.displayDensityState() === 'compact' ? 'gap-1.5' : '')
  )
  ...
  protected readonly selectClasses = computed(() =>
    mergeClasses(this.displayDensityState() === 'compact' ? 'w-[72px]' : 'w-[84px]')
  )

  protected readonly buttonSize = computed(() => (this.displayDensityState() === 'compact' ? 'icon-xs' : 'icon-sm'))
