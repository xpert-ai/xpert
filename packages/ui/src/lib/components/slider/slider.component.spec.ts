...
@Component({
  imports: [ZardSliderComponent],
  template: ` <z-slider [min]="0" [max]="1" [step]="0.1" [showValueLabel]="true" [value]="0.30000000000000004" /> `
})
...
@Component({
  imports: [ZardSliderComponent],
  template: `
    <z-slider [min]="min" [max]="max" [value]="value" [showValueLabel]="true" (changeEnd)="onChangeEnd($event)" />
  `
})
class MissingBoundsHostComponent {
  min?: number
  max?: number
  value?: number
  readonly changeEnds: ZardSliderValue[] = []
...
describe('ZardSliderComponent', () => {
  ...
  it('falls back to finite values when numeric inputs are missing', async () => {
    const fixture = await TestBed.configureTestingModule({
      imports: [MissingBoundsHostComponent]
    }).createComponent(MissingBoundsHostComponent)
    ...
    expect(thumb.getAttribute('aria-valuetext')).not.toBe('NaN')
    expect(fixture.nativeElement.textContent).not.toContain('NaN')

    dispatchMouseEvent(host, 'mousedown', 10, 5)
    dispatchMouseEvent(document, 'mousemove', 40, 5)
    dispatchMouseEvent(document, 'mouseup', 40, 5)
    fixture.detectChanges()

    expect(fixture.componentInstance.changeEnds.at(-1)).toBe(40)
  })
