...
@Component({
  ...
  template: `
    @if (!projectedItems().length) {
      @for (item of items(); track $index) {
        <z-toggle-group-item [value]="item.value" [disabled]="item.disabled" [aria-label]="item.ariaLabel || ''">
          @if (item.icon) {
  ...
  host: {
    ...
    '[class]': 'classes()',
    '[style.--z-toggle-group-item-checked-bg]': '"var(--primary)"',
    '[style.--z-toggle-group-item-checked-color]': '"var(--primary-foreground)"',
    role: 'group'
  },
