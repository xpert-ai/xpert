...
import { ZardPopoverComponent } from '../popover'
...
import { normalizeTagSelectToken, tokenizePastedValue, tokenizeTypedValue } from './use-tokenization'
...
export class ZardTagSelectComponent implements ControlValueAccessor {
  ...
  private commitTexts(values: readonly string[]): void {
    ...
    for (const rawValue of values) {
      ...
      nextValues = addTagSelectValue(nextValues, createdValue, this.compareWith(), typeof createdValue === 'string')
    }
    ...
    this.overlayRef
      ...
      .pipe(
        filter((event) => {
          ...
          return !this.triggerRef().nativeElement.contains(target) && !this.overlayRef?.overlayElement.contains(target)
        }),
