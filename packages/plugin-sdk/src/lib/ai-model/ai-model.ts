...
import type { AIGCModelClient, AsyncAIGCModelClient } from './types/aigc'
import { CommonParameterRules, IAIModel, TChatModelOptions } from './types/model'
import { ModelProfile } from './types/profile'
import { calculateModelPrice, isModelUsagePricingConfig, resolveModelUsagePricingSnapshot } from './pricing'
...
export abstract class AIModel implements IAIModel {
  ...
  getAIGCModel(copilotModel: ICopilotModel, options?: TChatModelOptions): AIGCModelClient | AsyncAIGCModelClient {
    void copilotModel
    void options
    throw new Error(`Unsupported AIGC model!`)
  }
  ...
  getPrice(
    ...
    if (!modelSchema || !modelSchema.pricing || isModelUsagePricingConfig(modelSchema.pricing)) {
      return {
        unitPrice: 0,
        unit: 0,
        totalAmount: 0,
