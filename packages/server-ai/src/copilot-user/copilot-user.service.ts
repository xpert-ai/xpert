...
export class CopilotUserService extends TenantOrganizationAwareCrudService<CopilotUser> {
     ...
     */
    async upsert(user: Partial<CopilotUser>, manager?: EntityManager): Promise<ICopilotUser> {
        const repository = manager?.getRepository(CopilotUser) ?? this.repository
        ...
        if (existing) {
            existing.tokenUsed = (existing.tokenUsed ?? 0) + (user.tokenUsed ?? 0)
            existing.priceUsed = Number(existing.priceUsed ?? 0) + Number(user.priceUsed ?? 0)
            existing.tokenLimit ??= user.tokenLimit
            existing.priceLimit ??= user.priceLimit
    ...
    async getUsageSummary(
        ...
        const query = (manager?.getRepository(CopilotUser) ?? this.repository)
