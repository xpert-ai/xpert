...
export class XpertAgentVariablesHandler implements IQueryHandler<XpertAgentVariablesQuery> {
        ...
        public async execute(command: XpertAgentVariablesQuery): Promise<TWorkflowVarGroup[]> {
                ...
                // All agents output
                // Pure workflow mode - when primary agent is hidden, use chat trigger as entry point
                let triggerNodeKeyForInput: string = null
                if (xpert.agent?.options?.hidden && xpert.agent?.key === nodeKey) {
                        const chatStarters = getWorkflowTriggers(_graph, 'chat')
                        if (chatStarters.length) {
                                // For input type, keep track of trigger to include its parameters as input variables
                                if (type === 'input') {
                                        triggerNodeKeyForInput = chatStarters[0].key
                                }
                                nodeKey = chatStarters[0].key
                        }
                }
                ...
                for await (const graph of graphsForVariables) {
                        ...
                        // Workflow nodes
                        // Note: When primary agent is hidden and we're fetching input variables (triggerNodeKeyForInput is set),
                        // we need to include the trigger node so its parameters are available as input variables
                        const workflowNodes = graph?.nodes?.filter(
                                (_) => _.type === 'workflow' && (_.entity.type === WorkflowNodeTypeEnum.SOURCE || _.key !== nodeKey || _.key === triggerNodeKeyForInput)
                        )
                        if (workflowNodes) {
