...
export class XpertAgentChatHandler implements ICommandHandler<XpertAgentChatCommand> {
    ...
    public async execute(command: XpertAgentChatCommand): Promise<Observable<MessageEvent>> {
        ...
        return new Observable<MessageEvent>((subscriber) => {
            ...
            RunnableLambda.from(async (state: TXpertChatState) => {
                let status = XpertAgentExecutionStatusEnum.SUCCESS
                let error = null
                let result = ''
                const agentStream = await this.commandBus.execute<
                    XpertAgentInvokeCommand,
                    Observable<string | TMessageContentComplex>
                >(
