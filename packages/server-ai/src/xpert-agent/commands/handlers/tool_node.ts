...
export class ToolNode<T = any> extends Runnable<T, T> {
    ...
    protected async run(input: any, config: RunnableConfig): Promise<T> {
        const messages = Array.isArray(input) ? input : this.channel ? input[this.channel]?.messages : input.messages
                            ...
                            { ...request.toolCall, type: 'tool_call' },
                            {
                                ...
                                metadata: {
                                    ...
                                    ...(this.toolDisplayTitle
                                        ? {
                                              displayTitle: this.toolDisplayTitle,
                                              toolDisplayTitle: this.toolDisplayTitle
                                          }
                                        : {}),
                                    ...(this.toolDisplayMessage
                                        ? {
                                              displayMessage: this.toolDisplayMessage,
                                              toolDisplayMessage: this.toolDisplayMessage
                                          }
                                        : {})
                                },
                                configurable: {
    ...
    protected async _tracedInvoke(input: T, config?: Partial<RunnableConfig>, runManager?: CallbackManagerForChainRun) {
