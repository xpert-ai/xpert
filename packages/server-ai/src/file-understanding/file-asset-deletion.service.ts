...
@Injectable()
export class FileAssetDeletionService {
    constructor(
    ...
    async delete(fileAssetId: string): Promise<void> {
        const { asset } = await this.fileAssetAccessService.resolve({
            locator: { fileAssetId },
            authority: { kind: 'current-owner' },
            operation: 'delete'
        })
        await this.cleanup(asset)
    }

    /** Dedicated project lifecycle entry; never trusts the supplied project ID as authority. */
    async purgeProjectFile(fileAssetId: string, projectId: string): Promise<void> {
        const { project } = await this.projectAccessService.assertCanPurge(projectId)
        const { asset } = await this.fileAssetAccessService.resolve({
            locator: { fileAssetId },
            authority: { kind: 'current-owner' },
            operation: 'read'
        })
        if (asset.projectId !== project.id || asset.tenantId !== project.tenantId) {
            throw new ForbiddenException(
                t('server-ai:Error.ProjectPurgeFileMismatch', {
                    defaultValue: 'File does not belong to the Project being permanently deleted.'
                })
            )
        }
        await this.cleanup(asset)
    ...
    private async cleanup(asset: FileAsset): Promise<void> {
        const fileAssetId = asset.id
        const pageImageStorageKeys = await this.listPageImageStorageKeys(fileAssetId)
        const storageProvider = await this.resolveStorageProvider(asset)

        await this.fileVectorService.deleteFileVectors(fileAssetId, asset)
        await this.fileCitationAnchorRepository.delete({ fileAssetId: fileAssetId })
        await this.fileEmbeddingRepository.delete({ fileAssetId: fileAssetId })
        await this.fileChunkRepository.delete({ fileAssetId: fileAssetId })
        await this.fileArtifactRepository.delete({ fileAssetId: fileAssetId })
        await this.conversationFileLinkRepository.delete({ fileAssetId: fileAssetId })
        await this.fileAssetRepository.delete({ id: fileAssetId })
