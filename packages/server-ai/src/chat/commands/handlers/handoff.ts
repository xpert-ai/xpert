...
const createHandoffTool = ({
    ...
    title: string
    description: string
    onHandoff?: (input: { taskId?: string; config: unknown }) => Promise<string | void>
}) => {
    /**
    ...
    if (onHandoff) {
        toolDescription += ' When this is part of a project task, include the taskId so execution context is recorded.'
    }
    const handoffTool = tool(
        async (input, config) => {
             ...
             */
            const executionId = await onHandoff?.({ taskId: input?.taskId, config })
            const toolMessage = new ToolMessage({
                content: `Successfully transferred to ${agentName}${executionId ? ` (execution ${executionId})` : ''}`,
                name: toolName,
        ...
        },
        {
            ...
            schema: z.object({
                ...
                taskId: z.string().optional().describe(`Project task id to associate with this delegation`)
            }),
...
function createHandoffBackMessages(agentName: string, supervisorName: string): [AIMessage, ToolMessage] {
    /**
