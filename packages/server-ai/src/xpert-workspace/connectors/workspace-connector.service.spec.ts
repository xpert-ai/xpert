...
import { Integration, RequestContext } from '@xpert-ai/server-core'
...
describe('XpertWorkspaceConnectorService', () => {
    let connectors: InMemoryRepository<XpertWorkspaceConnector>
    let sessions: InMemoryRepository<XpertWorkspaceConnectorOAuthSession>
    let service: XpertWorkspaceConnectorService
	let strategy: ConnectorStrategy
	let createIntegration: jest.Mock<Promise<Integration>, [CreateIntegrationInput]>
	let findIntegration: jest.Mock<Promise<Integration>, [string, string]>
	let findAllIntegrations: jest.Mock<Promise<{ items: Integration[]; total: number }>, [unknown?]>

    beforeEach(() => {
        connectors = new InMemoryRepository<XpertWorkspaceConnector>()
        sessions = new InMemoryRepository<XpertWorkspaceConnectorOAuthSession>()
        createIntegration = jest.fn(async (input: CreateIntegrationInput) => {
            const integration = new Integration()
            integration.id = 'created-integration'
            integration.name = input.name
            integration.slug = 'created-integration'
            integration.tags = []
            integration.provider = input.provider
            integration.options = input.options
            return integration
        })
		findIntegration = jest.fn().mockResolvedValue({
			id: 'integration-1',
			provider: 'example_app',
            options: {
		...
		findAllIntegrations = jest.fn().mockResolvedValue({
			items: [
				integrationFixture('integration-1', 'Accepted App', 'example_app', { accepted: true }),
				integrationFixture('integration-2', 'Rejected App', 'example_app', { accepted: false })
			],
			total: 2
		})
        strategy = {
        ...
        service = new XpertWorkspaceConnectorService(
            connectors,
            sessions,
            {
			...
			{
				findOneByIdWithinTenant: findIntegration,
				create: createIntegration,
				findAll: findAllIntegrations
			} as any,
            {
	...
	it('rejects app integrations that do not match the connector app provider', async () => {
        findIntegration.mockResolvedValueOnce({
            id: 'integration-1',
            provider: 'other_app',
            options: {
                appId: 'wrong_app_id',
                appSecret: 'wrong_app_secret'
            }
        } as Integration)
        ...
        await expect(
            service.startOAuth('workspace-1', 'example', {
                ...
                redirectUri: 'https://xpert.test/api/xpert-workspace/connectors/oauth/callback'
            })
	...
	it('returns only app integration options accepted by the connector strategy', async () => {
		;(strategy as any).isSelectableAppIntegration = jest.fn(
			(integration: Integration) => integration.options?.accepted !== false
		)

		await expect((service as any).appIntegrationSelectOptions('workspace-1', 'example')).resolves.toEqual([
			{
				value: 'integration-1',
				label: 'Accepted App',
		...
		expect(findAllIntegrations).toHaveBeenCalledWith({ where: { provider: 'example_app' } })
		expect((strategy as any).isSelectableAppIntegration).toHaveBeenCalledTimes(2)
	})

	it('rejects app integrations not accepted by the connector strategy', async () => {
		findIntegration.mockResolvedValueOnce(
			integrationFixture('integration-2', 'Rejected App', 'example_app', {
				accepted: false
			})
		)
		;(strategy as any).isSelectableAppIntegration = jest.fn(
		...
		await expect(
			service.startOAuth('workspace-1', 'example', {
				appIntegrationId: 'integration-2',
				redirectUri: 'https://xpert.test/api/xpert-workspace/connectors/oauth/callback'
			})
		).rejects.toBeInstanceOf(BadRequestException)
    ...
    it('does not expose unannotated Object dependencies to Nest injection', () => {
        const paramTypes = ((Reflect as any).getMetadata(PARAMTYPES_METADATA, XpertWorkspaceConnectorService) ??
            []) as unknown[]
    ...
    it('creates connector app integrations from strategy definition metadata', async () => {
        await service.startOAuth('workspace-1', 'example', {
            app: {
	...
	it('starts a connector flow without app integration when the strategy manages credentials', async () => {
        ...
        expect(start.connector.status).toBe('pending')
        expect(connectors.items[0].appIntegrationId).toBeNull()
        expect(sessions.items[0].appIntegrationId).toBeNull()
	...
	it('allows connector-managed credentials when app integrations are optional', async () => {
		strategy.definition = {
			provider: 'managed-example',
			label: 'Managed Example',
			appIntegrationProvider: 'managed-example_app',
			appIntegrationOptional: true,
			auth: { type: 'oauth2' }
		} as any
		;(strategy.buildAuthorizationUrl as jest.Mock).mockResolvedValueOnce({
			authorizationUrl: 'https://oauth.example.com/managed?state=state-1',
    ...
    it('rejects app integration ids when the connector definition does not declare app integration support', async () => {
        strategy.definition = {
            provider: 'managed-example',
            label: 'Managed Example',
            auth: { type: 'oauth2' }
...
function integrationFixture(id: string, name: string, provider: string, options: Record<string, unknown>) {
	const integration = new Integration()
	integration.id = id
