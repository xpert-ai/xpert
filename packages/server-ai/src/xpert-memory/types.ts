...
export type MemoryDirectReplyResult = {
    content: string
...
export interface MemoryAgentBridge {
    ...
    search(input: { query: string; limit?: number }): Promise<MemorySearchResult[]>
    reply(input: { query: string }): Promise<MemoryDirectReplyResult | null>
    write(input: {
...
export type AgentInternalCapabilities = {
    memory?: MemoryAgentBridge | null
}
