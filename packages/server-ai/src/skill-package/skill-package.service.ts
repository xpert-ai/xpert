...
export class SkillPackageService extends XpertWorkspaceBaseService<SkillPackage> {
    ...
    async installGithubSkillPackages(workspaceId: string, input: InstallGithubSkillPackagesInput) {
        ...
        for (const index of indexes) {
            ...
            packages.push(
                await this.installDirectGithubSkillPackage(workspaceId, hydratedIndex, installDir, normalizedInput)
            )
        }
    ...
    async syncTemplateSkillBundle(
        ...
        const packagePath =
            existingSkillPackage?.packagePath ??
            (await this.allocateWorkspaceSkillPackagePath(installDir, skillBaseName))
        const absolutePackagePath = join(installDir, packagePath)
    ...
    async getSkillPackageFileDownload(
        workspaceId: string,
        id: string,
        filePath: string
    ): Promise<SkillPackageDownloadTarget> {
        const { absolutePath, relativePath } = await this.resolveSkillPackageFilePath(workspaceId, id, filePath)
        const stat = await fs.stat(absolutePath).catch(() => null)
        if (!stat) {
            throw new BadRequestException('Skill file not found')
    ...
    async getSkillPackageRootDownload(workspaceId: string, id: string): Promise<SkillPackageDownloadTarget> {
        const { rootPath } = await this.resolveSkillPackageRoot(workspaceId, id)
        ...
        return {
            ...
            fileName: `${basename(rootPath)}.zip`,
            mimeType: 'application/zip',
            type: 'directory'
        }
    }
    ...
    private buildMetadataFromIndex(index: {
        ...
        const publisherName =
            index.publisher?.displayName?.trim() || index.publisher?.name?.trim() || index.publisher?.handle?.trim()
    ...
    private buildDirectGithubRepository(input: NormalizedGithubSkillInstallInput, tenantId: string): ISkillRepository {
        const organizationId = RequestContext.getOrganizationId()
    ...
    private async installDirectGithubSkillPackage(
        ...
        try {
            ...
            const packagePath = this.normalizePackagePath(
                await this.skillSourceProviderRegistry
                    .get(GITHUB_SKILL_SOURCE_PROVIDER)
                    .installSkillPackage(index, installDir),
                installDir
            )
            if (!packagePath) {
    ...
    private normalizeSharedSkillInput(
        skillPackage: SkillPackage,
        input: IShareSkillPackageInput
    ): SharedSkillMetadataInput {
        const fallbackDisplayName =
            readI18nText(skillPackage.metadata?.displayName) || skillPackage.name || skillPackage.metadata?.name || ''
    ...
    private async publishSharedSkillPackage(
        ...
        const sharedPath = getOrganizationSharedSkillPath(
            options.tenantId,
            options.organizationId,
            options.sharedSkillId
        )
        const publisher =
    ...
    private shouldRefreshInstalledMarketplaceSkillPackage(
        ...
        const installedVersion =
            typeof skillPackage.metadata?.version === 'string' ? skillPackage.metadata.version.trim() : ''
    ...
    private async findOrCreateScopeDefaultWorkspace(
        tenantId: string,
        ...
        const workspaceName = organizationId
            ? getDefaultOrganizationWorkspaceName()
            : DEFAULT_TENANT_SKILL_WORKSPACE_NAME
...
function normalizeGithubRepositoryPath(value?: string) {
    return normalizeOptionalString(value).replace(/\\/g, '/').replace(/^\/+/, '').replace(/\/+$/, '')
}
...
function buildDirectGithubRepositoryId(input: NormalizedGithubSkillInstallInput) {
    return ['github-direct', input.repositoryUrl, input.repositoryPath, input.branch].filter(Boolean).join(':')
}
...
function readDirectGithubSkillSource(
    metadata?: Partial<SkillMetadata> | null
): NormalizedGithubSkillInstallSource | null {
...
function parseWorkspaceSkillFrontmatter(
    ...
    if (options?.allowedKeys?.length) {
        ...
        const unexpectedKeys = Object.keys(parsedFrontmatter as Record<string, unknown>).filter(
    ...
    return {
        ...
        description,
        version:
            typeof candidate.version === 'string' && candidate.version.trim() ? candidate.version.trim() : undefined,
        license:
            typeof candidate.license === 'string' && candidate.license.trim() ? candidate.license.trim() : undefined,
...
function prepareWorkspaceSkillPackageFiles(
    files?: WorkspaceSkillPackageFileInput[]
): PreparedWorkspaceSkillPackageFile[] {
    const inputs = files ?? []
    if (inputs.length > MAX_WORKSPACE_SKILL_PACKAGE_FILES) {
        throw new BadRequestException(
            `Workspace skill packages can include at most ${MAX_WORKSPACE_SKILL_PACKAGE_FILES} bundled files`
        )
    }
