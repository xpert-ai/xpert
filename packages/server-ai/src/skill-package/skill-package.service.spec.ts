...
describe('SkillPackageService', () => {
    ...
    it('returns zip download metadata for workspace skill roots', async () => {
        tempRoot = await mkdtemp(join(tmpdir(), 'skill-package-download-root-'))
        const skillRoot = join(tempRoot, 'weather')
        await mkdir(skillRoot, { recursive: true })
        await writeFile(join(skillRoot, 'SKILL.md'), '# Weather\n', 'utf8')
        ;(getWorkspaceSkillsRoot as jest.Mock).mockReturnValue(tempRoot)
        ...
        const result = await service.getSkillPackageRootDownload('workspace-1', 'skill-1')
    ...
    it('rejects file downloads without a file path', async () => {
        tempRoot = await mkdtemp(join(tmpdir(), 'skill-package-download-empty-path-'))
        const skillRoot = join(tempRoot, 'weather')
        await mkdir(skillRoot, { recursive: true })
        await writeFile(join(skillRoot, 'SKILL.md'), '# Weather\n', 'utf8')
        ;(getWorkspaceSkillsRoot as jest.Mock).mockReturnValue(tempRoot)
        ...
        await expect(service.getSkillPackageFileDownload('workspace-1', 'skill-1', '')).rejects.toThrow(
