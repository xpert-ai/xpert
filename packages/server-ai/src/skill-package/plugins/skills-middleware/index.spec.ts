...
describe('SkillsMiddleware', () => {
    ...
    it('filters disabled configured skills before building the prompt section', async () => {
        ...
        const loadSkillMetadata = jest
            .spyOn(middleware as any, 'loadSkillMetadata')
            .mockImplementation(async (_workspaceRoot: string, skillIds: string[], workspaceId: string) =>
    ...
    it('loads all workspace skills in blacklist mode when selectedSkillIds are absent', async () => {
        ...
        jest.spyOn(middleware as any, 'parseSkillPackage').mockImplementation(
            async (_workspaceRoot: string, skillPackage: any) => ({
                id: skillPackage.id,
    ...
    it('loads repository-backed defaults, removes config blacklisted skills, and still applies runtime disabled skills last', async () => {
        ...
        expect(loadSkillMetadata).toHaveBeenCalledWith(runtimeSkillsRoot, ['skill-local-extra'], 'workspace-1')
    ...
    it('schedules the same skill selection again when the sandbox binding changes', async () => {
        const middleware = createMiddleware()
        const copyA = deferred<{ ok: true }>()
        const copyB = deferred<{ ok: true }>()
        const execute = jest.fn().mockReturnValueOnce(copyA.promise).mockReturnValueOnce(copyB.promise)
        ;(middleware as any).commandBus = { execute }
        jest.spyOn(middleware as any, 'loadSkillMetadata').mockResolvedValue([
            {
                id: 'skill-a',
                ...
                configurable: {
                    sandbox: {
                        ...
                        workspaceBinding: {
                            ...
                            workspaceRoot: '/workspace/runtime',
        ...
        await instance.wrapModelCall(baseRequest, handler)
        await instance.wrapModelCall(nextRequest, handler)

        expect(execute).toHaveBeenCalledTimes(2)
        copyA.resolve({ ok: true })
        copyB.resolve({ ok: true })
        await Promise.resolve()
    })
    ...
    it('keeps a failed skill sync visible for later skill file reads', async () => {
        const middleware = createMiddleware()
        const syncError = new Error('copy failed')
        const execute = jest.fn().mockRejectedValue(syncError)
        const readExecute = jest.fn().mockResolvedValue({
            exitCode: 0,
            output: 'stale instructions'
        })
        ;(middleware as any).commandBus = { execute }
        jest.spyOn(middleware as any, 'loadSkillMetadata').mockResolvedValue([
            {
