...
const skillPackageFileSchema = z
    .object({
        path: z
            .string()
            .describe(
                'POSIX relative path under agents/openai.yaml, scripts/, references/, or assets/. Do not include SKILL.md.'
            ),
        content: z
            .string()
            .optional()
            .describe('UTF-8 text file content. Use for agents/openai.yaml, scripts, and references.'),
...
export class SkillCreatorMiddleware implements IAgentMiddlewareStrategy {
    ...
    createMiddleware(_options: unknown, context: IAgentMiddlewareContext): PromiseOrValue<AgentMiddleware> {
        return {
            name: SKILL_CREATOR_MIDDLEWARE_NAME,
            tools: [
                this.createWorkspaceSkillTool(context),
                this.getWorkspaceSkillForEditTool(context),
                this.updateWorkspaceSkillTool(context),
                this.deleteWorkspaceSkillTool(context)
            ],
    ...
    private createWorkspaceSkillTool(context: IAgentMiddlewareContext) {
        return tool(
            async (input) => {
            ...
            },
            {
                ...
                schema: z.object({
                    ...
                    skillName: z
                        .string()
                        .optional()
                        .describe('Optional stable skill name to use for package path allocation.'),
                    skillMarkdown: z
                        .string()
    ...
    private getWorkspaceSkillForEditTool(context: IAgentMiddlewareContext) {
        return tool(
            async (input) => {
                const workspaceId = this.getWorkspaceId(context)
                if (!workspaceId) {
    ...
    private updateWorkspaceSkillTool(context: IAgentMiddlewareContext) {
        return tool(
            async (input) => {
                const workspaceId = this.getWorkspaceId(context)
                if (!workspaceId) {
            ...
            },
            {
                ...
                description:
                    'Replace the SKILL.md for one workspace-authored skill package. The skillRef must uniquely match one package.',
                schema: z.object({
                    skillRef: skillRefSchema,
                    skillMarkdown: z
                        .string()
    ...
    private deleteWorkspaceSkillTool(context: IAgentMiddlewareContext) {
        return tool(
            async (input) => {
                const workspaceId = this.getWorkspaceId(context)
                if (!workspaceId) {
            ...
            },
            {
                ...
                description:
                    'Delete one workspace-authored skill package. The skillRef must uniquely match one package.',
                schema: z.object({
                    skillRef: skillRefSchema
                })
            }
    ...
    private getWorkspaceId(context: IAgentMiddlewareContext) {
        return context.workspaceId?.trim() || null
    }

    private missingWorkspaceResult() {
        return {
            status: 'rejected',
