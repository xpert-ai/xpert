...
export class DefaultTransformerStrategy implements IDocumentTransformerStrategy<TDefaultTransformerConfig> {
    ...
    async processEpub(url: string): Promise<Document<ChunkMetadata>[]> {
    ...
    async processExcel(url: string): Promise<Document<ChunkMetadata>[]> {
    ...
    async processOpenDocument(url: string): Promise<Document<ChunkMetadata>[]> {
        const loader = new PPTXLoader(url)
        return toChunkDocuments(await loader.load())
    }

    async processImage(name = 'image', url: string): Promise<Document<ChunkMetadata>[]> {
        return [
            new Document<ChunkMetadata>({
...
function hasNodeErrorCode(error: unknown, code: string): error is { code: string } {
