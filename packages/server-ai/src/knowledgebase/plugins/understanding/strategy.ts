                ...
                return await wrapAgentExecution(
                    async () => {
                        ...
                        if (isTest) {
                            ...
                            documents = task.context.documents.filter((doc) =>
                        ...
                        const strategy = await this.queryBus.execute<
                            KnowledgeStrategyQuery,
                        ...
                        const strategyConfig = {
                            ...(entity.config ?? {}),
                            stage: isDraft ? ('test' as const) : ('prod' as const)
                        }
                        const requiresVisionModel = (await strategy.requiresVisionModel?.(strategyConfig)) ?? true
                        const visionModel = requiresVisionModel
                            ? await this.knowledgebaseService.getVisionModel(knowledgebaseId, entity.visionModel)
                            : undefined
