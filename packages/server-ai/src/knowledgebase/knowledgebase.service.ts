...
export class KnowledgebaseService extends XpertWorkspaceBaseService<Knowledgebase> {
        ...
        async getVisionModel(knowledgebaseId: string, visionModel: TCopilotModel) {
                ...
                // If copilotId is set but copilot object is not loaded, fetch it
                let copilot = visionModel?.copilot
                if (!copilot && visionModel?.copilotId) {
                        copilot = await this.queryBus.execute(new CopilotGetOneQuery(RequestContext.currentTenantId(), visionModel.copilotId, ['modelProvider']))
                }
