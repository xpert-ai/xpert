...
export async function closeToolsets(toolsets: _BaseToolset[], onError: (error: unknown) => void) {
    await Promise.all(toolsets.map((toolset) => toolset.close().catch(onError)))
}

export abstract class BaseTool<T extends ZodObjectAny = ZodObjectAny> extends StructuredTool<T> {
    schema: any = z.object({ input: z.string().optional() }).transform((obj) => obj.input)
}
