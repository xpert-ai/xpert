...
export class RunnableCallable<I = unknown, O = unknown> extends Runnable<I, O> {
    ...
    protected async _tracedInvoke(input: I, config?: Partial<RunnableConfig>, runManager?: CallbackManagerForChainRun) {
    ...
    async invoke(
        ...
        if (this.trace) {
            ...
            returnValue = await AsyncLocalStorageProviderSingleton.runWithConfig(mergedConfig, async () =>
...
export function normalizeChatState(
    value?: unknown,
    humanInput?: TChatRequestHuman | string,
    path = 'state'
): TXpertChatState {
    ...
    const state = value ? ({ ...(value as Record<string, unknown>) } as TXpertChatState) : ({} as TXpertChatState)
    const normalizedHumanInput = normalizeHumanInput(
        state[STATE_VARIABLE_HUMAN] ?? humanInput ?? {},
        `${path}.${STATE_VARIABLE_HUMAN}`
    )

    return {
        ...state,
        [STATE_VARIABLE_HUMAN]: normalizedHumanInput
    }
}

export function normalizeHumanInput(value: unknown, path: string): TChatRequestHuman {
    ...
    if (isRecord(value)) {
        return value as TChatRequestHuman
    }
