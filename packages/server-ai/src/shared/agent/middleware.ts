...
const DEPRECATED_MEMORY_MIDDLEWARE = 'LongTermMemoryMiddleware'
...
export async function getAgentMiddlewares(
    ...
    const selectedExclusiveCategories = new Set<string>()
    for (const middlewareNode of middlewares) {
        ...
        if (provider === DEPRECATED_MEMORY_MIDDLEWARE) {
            console.warn(
                `Deprecated middleware "${DEPRECATED_MEMORY_MIDDLEWARE}" ignored. Use the built-in file memory middleware instead.`
            )
            continue
        }
        ...
        const exclusiveCategory = strategy.meta?.exclusiveCategory
        if (exclusiveCategory && selectedExclusiveCategories.has(exclusiveCategory)) {
            console.warn(
                `Middleware "${provider}" ignored because another middleware in exclusive category "${exclusiveCategory}" is already active.`
            )
            continue
        }
        ...
        if (middleware) {
            ...
            if (exclusiveCategory) {
                selectedExclusiveCategories.add(exclusiveCategory)
            }
        }
    }
