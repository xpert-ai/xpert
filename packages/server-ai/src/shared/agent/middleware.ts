...
function isSkillsMiddlewareNode(node?: TXpertTeamNode | null) {
    ...
    return node?.type === 'workflow' && normalizeMiddlewareProvider(entity?.provider) === SKILLS_MIDDLEWARE_NAME
}

function assertSingleSkillsMiddlewareConnection(sourceKey: string, middlewareNodes: TXpertTeamNode[]) {
    const skillsMiddlewareCount = middlewareNodes.filter(isSkillsMiddlewareNode).length
...
export function getRuntimeEnabledMiddlewareNodes(
    ...
    const runtimeEnabledMiddlewares = middlewares.filter((middlewareNode) => {
    ...
    return runtimeEnabledMiddlewares
}
