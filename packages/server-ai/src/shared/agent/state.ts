...
import { TRuntimeCapabilitiesSelectionWithRecommended } from './runtime-capabilities'
    ...
    return {
        ...
        ...Object.keys(state).reduce((acc, key) => {
            ...
            if (Array.isArray(value)) {
                acc[key] =
                    key === 'messages'
                        ? getBufferString(value as BaseMessage[])
...
export function stateVariable(variable: TStateVariable) {
    ...
    try {
        defaultValue = isFunction(variable.default)
            ? variable.default()
            : [XpertParameterTypeEnum.STRING, XpertParameterTypeEnum.TEXT, XpertParameterTypeEnum.PARAGRAPH].includes(
                    variable.type
                )
              ? variable.default
...
export function findChannelByTool(
    values: typeof AgentStateAnnotation.State,
    toolName: string
): [string, TMessageChannel] {
            ...
            if (
                channel.messages?.find(
                    (message) =>
                        ...
                        message.tool_calls?.find((_) => _.name === toolName)
                )
            ) {
                return true
            }
...
export async function updateToolCalls(
    ...
    if (lastMessage.id) {
        ...
        await graph.updateState({ configurable: config }, { [channel]: { messages: [newMessage] } }, command.agentKey)
    }
}
