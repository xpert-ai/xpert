...
export class XpertTableService extends TenantOrganizationAwareCrudService<XpertTable> {
         ...
         */
        async queryRows(tableId: string, options: {
                columns?: string[]
                ...
                try {
                        ...
                        if (options.orderBy && options.orderBy.length > 0) {
                                orderByStr = options.orderBy
                                        .map(o => `${o.column} ${o.direction || 'ASC'}`)
                                        .join(', ')
                        }

                        return await adapter.tableDataOp(DBTableDataAction.SELECT, {
                                schema: table.schema || undefined,
                                table: table.name,
                                columns: options.columns?.map(col => ({ name: col, fieldName: col })),
                                where: options.where,
                                orderBy: orderByStr,
                                limit: options.limit
                        })
                } catch (error) {
                        this.#logger.error(`Error querying rows: ${getErrorMessage(error)}`)
                        throw error
                } finally {
                        adapter.teardown()
                }
         ...
         */
        async updateRows(tableId: string, options: {
                columns: { name: string; value: any; type: string }[]
                where?: Record<string, any>
        }) {
                const table = await this.findOneByIdString(tableId)
                if (table.status !== XpertTableStatus.ACTIVE) {
                ...
                try {
                        ...
                        for (const col of options.columns) {
                                setValues[col.name] = col.value
                        }

                        const result = await adapter.tableDataOp(DBTableDataAction.UPDATE, {
                                schema: table.schema || undefined,
                                table: table.name,
                                set: setValues,
                                where: options.where
                        ...
                        this.#logger.error(`Error updating rows: ${getErrorMessage(error)}`)
                        throw error
                } finally {
                        adapter.teardown()
                }
         ...
         */
        async deleteRows(tableId: string, where?: Record<string, any>) {
                const table = await this.findOneByIdString(tableId)
                if (table.status !== XpertTableStatus.ACTIVE) {
                        throw new BadRequestException(`Table ${table.name} is not active.`)
                }
                ...
                try {
                        const result = await adapter.tableDataOp(DBTableDataAction.DELETE, {
                                schema: table.schema || undefined,
                                table: table.name,
                                deleteWhere: where
                        })
                        ...
                        this.#logger.error(`Error deleting rows: ${getErrorMessage(error)}`)
                        throw error
                } finally {
                        adapter.teardown()
                }
