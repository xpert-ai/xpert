...
class FakeEmbeddings {
    async embedDocuments(documents: string[]) {
        return documents.map((document) => [document.length, document.includes('king') ? 1 : 0])
    }

    async embedQuery(document: string) {
        return [document.length, document.includes('king') ? 1 : 0]
    }
}
...
describe('batchSearchOps', () => {
    ...
    it('should handle batch search operations without embeddings', async () => {
        ...
        expect(results[0]).toEqual(
            expect.objectContaining({ key: 'key1', value: expect.objectContaining({ data: 'value1' }) })
        )
    })
})
