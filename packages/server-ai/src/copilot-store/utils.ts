 ...
 */
export function compareValues(itemValue: unknown, filterValue: unknown): boolean {
...
export function getTextAtPath(obj: unknown, path: string[] | string): string[] {
    ...
    function extractFromObj(obj: unknown, tokens: string[], pos: number): string[] {
        ...
        if (token.startsWith('[') && token.endsWith(']')) {
            ...
            if (typeof obj === 'object' && obj !== null && token in obj) {
                results.push(...extractFromObj((obj as Record<string, unknown>)[token], tokens, pos + 1))
            }
        }
...
export function formatProfileMemory(item: SearchItem) {
  ...
  ${item.value.layerLabel ? `<layer>${item.value.layerLabel}</layer>` : ''}
  ${item.value.audience ? `<audience>${item.value.audience}</audience>` : ''}
