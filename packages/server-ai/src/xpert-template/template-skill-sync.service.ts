...
export class TemplateSkillSyncService {
    ...
    async syncCurrentTenantSkillAssets(
        input: CurrentTenantTemplateSkillSyncInput = {}
    ): Promise<ITemplateSkillSyncResult> {
        const tenantId = RequestContext.currentTenantId()
        if (!tenantId) {
            throw new BadRequestException('Tenant context is required to sync template skill assets')
        }
        ...
        const executeSync = async () => {
            ...
            await this.xpertTemplateService.invalidateSkillTemplateCaches()
            const featuredRefs = await this.validateSkillRefs(
                await this.xpertTemplateService.getSkillsMarketFeaturedRefs()
            )
            const workspaceDefaults = await this.validateSkillRefs(
                await this.xpertTemplateService.getBootstrapDefaultSkillRefs()
            )
    ...
    async reconcileTenantsFromTemplates() {
        ...
        for (const tenantId of tenantIds) {
            const lockAcquired = await this.withTenantSyncLock(tenantId, 'skip', async () => {
                await this.xpertTemplateService.invalidateSkillTemplateCaches()
                const fingerprint = await this.xpertTemplateService.calculateSkillAssetFingerprint()
                const previousFingerprint = await this.cacheManager.get<string>(
                    this.getTenantFingerprintCacheKey(tenantId)
                )
                ...
                if (!currentUser?.id) {
                    this.#logger.warn(
                        `Skip template skill sync for tenant '${tenantId}' because no admin user is available`
                    )
                    return true
                }
            ...
            if (lockAcquired === null) {
                this.#logger.debug(
                    `Skip template skill sync for tenant '${tenantId}' because another sync is already running`
                )
            }
        }
    }
    ...
    private async withTenantSyncLock<T>(
        ...
        const [lockRow] = await manager.query('SELECT pg_try_advisory_lock(hashtext($1), hashtext($2)) AS locked', [
            tenantId,
