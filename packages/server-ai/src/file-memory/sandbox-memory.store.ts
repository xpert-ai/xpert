...
export class XpertSandboxMemoryStore {
    ...
    resolvePath(...segments: Array<string | undefined | null>) {
        return normalizeFileMemoryRelativePath(
            path.posix.join(this.rootPath, ...segments.filter(Boolean).map((segment) => `${segment}`))
        )
    }
...
export function assertFileMemorySandboxFeatureEnabled(context: {
    xpertFeatures?: { sandbox?: { enabled?: boolean } } | null
}) {
