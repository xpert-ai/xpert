...
import { PLUGIN_RESOURCE_INSTALLATION_STATUS, XpertToolsetCategoryEnum } from '@xpert-ai/contracts'
...
describe('PluginResourceInstallerService helpers', () => {
    ...
    it('attaches template builtin toolset dependencies to the imported xpert draft', async () => {
        const { service, xpertService, toolsetRepo } = createInstaller({
            toolsets: [createSeedreamToolset()]
        })

        await service.installTemplate('@xpert-ai/plugin-canvas:canvas-assistant', 'workspace-1')
    ...
    it('rolls back imported template xpert when a required builtin toolset is missing', async () => {
        const { service, xpertService } = createInstaller({
            toolsets: []
        })

        await expect(
            service.installTemplate('@xpert-ai/plugin-canvas:canvas-assistant', 'workspace-1')
        ).rejects.toThrow(
...
function createInstaller(options?: { toolsets?: any[] }) {
    const importedXpert = {
        id: 'xpert-1',
        workspaceId: 'workspace-1',
        agent: {
