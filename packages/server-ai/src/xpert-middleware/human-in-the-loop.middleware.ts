...
export class HumanInTheLoopMiddleware implements IAgentMiddlewareStrategy {
    meta: TAgentMiddlewareMeta = {
        ...
        description: {
            ...
            zh_Hans:
                '允许您在智能体工具调用中添加人工监督。当模型提出可能需要审核的操作（例如，写入文件或执行 SQL）时，中间件可以暂停执行并等待决策。'
        },
        icon: {
            type: 'svg',
            ...
            properties: {
                ...
                descriptionPrefix: {
                    ...
                    description: {
                        ...
                        zh_Hans:
                            '用于构建面向人类的审批消息的前缀。提供有关正在审核的工具调用的上下文；不会更改基础操作。'
                    },
                    'x-ui': {
                        component: 'textarea',
                        span: 2
    ...
    createMiddleware(
        ...
        const processDecision = (
            ...
            if (decision.type === 'reject' && allowedDecisions.includes('reject')) {
                ...
                const content =
                    decision.message ?? `User rejected the tool call for \`${toolCall.name}\` with id ${toolCall.id}`
            ...
            const msg = `Unexpected human decision: ${JSON.stringify(
                ...
                toolCall.name
            }'. Expected one of ${JSON.stringify(allowedDecisions)} based on the tool's configuration.`
            throw new Error(msg)
        }
            ...
            afterModel: {
                ...
                hook: async (state, runtime) => {
                    ...
                    const jumpTo: JumpToTarget | undefined =
                        artificialToolMessages.length && !hasPendingToolCalls ? 'model' : undefined
                    return {
