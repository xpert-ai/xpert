...
export class ConnectorMiddleware implements IAgentMiddlewareStrategy<ConnectorMiddlewareConfig> {
    readonly meta: TAgentMiddlewareMeta = {
        ...
        description: {
            en_US: 'Select a connector and let its provider plugin prepare runtime credentials and tools.',
            zh_Hans: '选择连接器，由对应插件准备运行时凭证和工具。'
        },
        configSchema: {
            type: 'object',
            properties: {
                provider: {
                    ...
                    description: {
                        en_US: 'Connector provider.',
                        zh_Hans: '连接器提供方。'
                    },
                    'x-ui': {
                        component: 'remoteSelect',
                        selectUrl: '/api/connector/provider-options',
                        depends: [
                            {
                                source: 'context',
                                name: 'workspaceId'
...
function createMissingRuntimeMiddleware(runtimeProvider: string, cause: unknown): AgentMiddleware {
    const message =
        ...
        'Install or reload the provider plugin that implements this connector.'
