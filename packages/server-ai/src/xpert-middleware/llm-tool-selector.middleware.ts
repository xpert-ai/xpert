...
function createToolSelectionResponse(tools: StructuredToolInterface[]) {
    ...
    const toolEnum = z.union(toolLiterals as [z.ZodLiteral<string>, z.ZodLiteral<string>, ...z.ZodLiteral<string>[]])

    return z.object({
        tools: z.array(toolEnum).describe('Tools to use. Place the most relevant tools first.')
    })
}
...
export class LLMToolSelectorNameMiddleware implements IAgentMiddlewareStrategy {
    ...
    async createMiddleware(options: LLMToolSelectorConfig, context: IAgentMiddlewareContext): Promise<AgentMiddleware> {
        ...
        if (error) {
            throw new Error(`Invalid llmToolSelector middleware options: ${z4.prettifyError(error)}`)
        }
        ...
        return {
            ...
            async wrapModelCall(request, handler) {
                const selectionRequest = await prepareSelectionRequest(request, { ...options, model }, request.runtime)
                if (!selectionRequest) {
                ...
                const response = await context.runtime.wrapWorkflowNodeExecution(
                    async () => {
                        ...
                        if (!response || typeof response !== 'object' || !('tools' in response)) {
                            throw new Error(`Expected object response with tools array, got ${typeof response}`)
                        }
                        return {
...
>(
    ...
    const baseTools = request.tools.filter(
        (tool): tool is StructuredToolInterface =>
            typeof tool === 'object' && 'name' in tool && 'description' in tool && typeof tool.name === 'string'
    )
    ...
    const modelInstance = !model ? (request.model as BaseLanguageModel) : model
 ...
 */
function processSelectionResponse<TState extends Record<string, unknown> = Record<string, unknown>, TContext = unknown>(
    ...
    for (const toolName of response.tools) {
         ...
         */
        if (!selectedToolNames.includes(toolName) && (maxTools === undefined || selectedToolNames.length < maxTools)) {
            selectedToolNames.push(toolName)
        }
    }

    if (invalidToolSelections.length > 0) {
        throw new Error(`Model selected invalid tools: ${invalidToolSelections.join(', ')}`)
    }
