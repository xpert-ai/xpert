...
jest.mock('@xpert-ai/contracts', () => ({
    ...
    normalizeAssistantBindingToolPreferences: (value: unknown) => value,
...
describe('AssistantBindingService', () => {
    ...
    it('lists system bindings for an explicit organization scope', async () => {
        repository.find.mockResolvedValue([{ code: AssistantCode.CHAT_COMMON }])

        const result = await service.getScopedBindings(AssistantBindingScope.ORGANIZATION)

        expect(result).toEqual([{ code: AssistantCode.CHAT_COMMON }])
    ...
    it('rejects user scope for system assistants', async () => {
        await expect(service.getBinding(AssistantCode.CHAT_COMMON, AssistantBindingScope.USER)).rejects.toBeInstanceOf(
            BadRequestException
        )
    })
