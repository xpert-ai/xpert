  ...
  implements OnModuleInit
{
  ...
  private async saveBinding(
    existing: AssistantBinding | null,
    input: IAssistantBinding,
    userScoped: boolean
  ) {
    const currentUserId = RequestContext.currentUserId()
  ...
  private resolveSystemScopeContext(scope: SystemScope, requireOrganization: boolean): ScopeContext {
    ...
    const organizationId = requireOrganization ? RequestContext.requireOrganizationScope() : RequestContext.getOrganizationId()
    return {
      tenantId,
      organizationId: organizationId ?? null
    }
