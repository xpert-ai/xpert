...
export class XpertService extends TenantOrganizationAwareCrudService<Xpert> {
        ...
        async bulkAddManagers(id: string, userIds: string[]) {
                const xpert = await this.findOne(id, { relations: ['managers'] })
                if (!xpert) {
                        throw new NotFoundException(`Xpert with id ${id} not found`)
                }
                ...
                const existingManagerIds = new Set(xpert.managers.map((m) => m.id))
                
                // Filter out users that are already managers
                const newUserIds = userIds.filter((userId) => !existingManagerIds.has(userId))
                ...
                const { items: newUsers } = await this.userService.findAll({
                        where: { id: In(newUserIds) }
                })

                // Add new managers
                xpert.managers = [...xpert.managers, ...newUsers]
                await this.repository.save(xpert)
