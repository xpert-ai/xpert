...
export class GetXpertWorkflowHandler implements IQueryHandler<GetXpertWorkflowQuery> {
    ...
    public async execute(command: GetXpertWorkflowQuery): Promise<TXpertWorkflowQueryOutput> {
        ...
        if (draft && xpert.draft) {
            ...
            if (!agentNode) {
                throw new Error(
                    await this.i18nService.translate('xpert.Error.NoAgentInGraph', {
            ...
            if (!xpert.graph) {
                throw new Error(
                    await this.i18nService.translate('xpert.Error.NoGraph', {
                        lang: mapTranslationLanguage(RequestContext.getLanguageCode())
                    })
                )
            }
            ...
            if (agent) {
                const next = xpert.graph?.connections
                    ...
                    .map((conn) =>
                        xpert.graph.nodes.find(
                            (_) => (_.type === 'agent' || _.type === 'workflow') && _.key === conn.to
                        )
                    )
                    .filter((n) => !(n.type === 'workflow' && n.entity.type === WorkflowNodeTypeEnum.TASK))
