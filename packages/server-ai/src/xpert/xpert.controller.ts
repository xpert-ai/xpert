...
export class XpertController extends CrudController<Xpert> {
        ...
        @Post(':id/managers/bulk/upload')
        @UseInterceptors(
                FileInterceptor('file', {
                        storage: new FileStorage().storage({
                                dest: path.join('temp'),
                                prefix: 'managers-csv-upload'
                        })
                })
        )
        @ApiOperation({ summary: 'Upload and parse CSV file for bulk manager import' })
        @ApiResponse({
                status: HttpStatus.OK,
                description: 'Users parsed from CSV file'
        })
        async uploadAndParseManagersCsv(
                @Param('id') id: string,
                ...
                try {
                        ...
                        for (const encoding of encodingsToTry) {
                                try {
                                        ...
                                        if (isValidDecoding(decoded, encoding)) {
                                                if (containsValidChinese(decoded) || !looksLikeMisDecoded(decoded)) {
                                                        bestDecodedContent = decoded
                                                        bestEncoding = encoding
                                                        break
                                                }
                                                
                                                if (!bestDecodedContent || decoded.length > bestDecodedContent.length) {
                                                        bestDecodedContent = decoded
                                                        bestEncoding = encoding
                                                }
                                        }
                        ...
                        // Parse CSV content
                        const lines = bestDecodedContent.split(/\r?\n/).filter(line => line.trim())
                        if (lines.length < 2) {
                        ...
                        if (emailIndex === -1) {
                                throw new BadRequestException('CSV file must contain an "email" column')
                        }
                        ...
                        if (emails.length === 0) {
                                throw new BadRequestException('No valid email addresses found in CSV file')
                        }

                        // Find users by emails
                        const users = await this.service.findUsersByEmails(emails)
        ...
        @Put(':id/managers/bulk')
        @ApiOperation({ summary: 'Bulk add managers to xpert' })
        @ApiResponse({
                status: HttpStatus.OK,
                description: 'Managers added successfully'
        })
        async bulkAddManagers(@Param('id') id: string, @Body() userIds: string[]) {
                return this.service.bulkAddManagers(id, userIds)
        }
