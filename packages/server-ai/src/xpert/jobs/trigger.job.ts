...
import { runWithRequestContext as _runWithRequestContext, UserService } from '@metad/server-core'
...
export class XpertTriggerConsumer {
	...
	async process(job: Job<TTriggerJob>) {
		...
		runWithRequestContext({ user, headers: { ['organization-id']: xpert.organizationId, language: user.preferredLanguage } }, null, async () => {
			_runWithRequestContext({ user, headers: { ['organization-id']: xpert.organizationId, language: user.preferredLanguage } }, async () => {
				try {
					// Chat with xpert
