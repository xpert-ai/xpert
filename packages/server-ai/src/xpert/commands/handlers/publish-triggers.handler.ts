...
export class XpertPublishTriggersHandler implements ICommandHandler<XpertPublishTriggersCommand> {
    ...
    public async execute(command: XpertPublishTriggersCommand): Promise<void> {
        ...
        for (const changed of delta.changed) {
            ...
            try {
                ...
                const rollbackStatus = rollbackError ? `rollback="${getErrorMessage(rollbackError)}"` : 'rollback="ok"'
                this.#logger.error(
    ...
    private resolveProvider(
        ...
        try {
            ...
            this.#logger.warn(`Trigger provider "${providerName}" not found for xpert "${xpertId}", skip reconcile`)
            return null
        }
    }
