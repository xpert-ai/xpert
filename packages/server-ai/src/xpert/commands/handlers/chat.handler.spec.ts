...
describe('XpertChatHandler', () => {
    ...
    it('marks the execution and conversation as error when agent chat fails before streaming', async () => {
        ...
        const stream = await handler.execute(
            new XpertChatCommand(
                ...
                } as unknown as ConstructorParameters<typeof XpertChatCommand>[0],
                {
                    xpertId: 'xpert-1'
                }
            )
        ...
        const executionErrorCommand = commands.find(
            (command): command is XpertAgentExecutionUpsertCommand =>
                command instanceof XpertAgentExecutionUpsertCommand &&
                command.execution.status === XpertAgentExecutionStatusEnum.ERROR
        )
        const messageErrorCommand = commands.find(
            (command): command is ChatMessageUpsertCommand =>
                command instanceof ChatMessageUpsertCommand &&
                command.entity.status === XpertAgentExecutionStatusEnum.ERROR
        )
        const conversationErrorCommand = commands.find(
            (command): command is ChatConversationUpsertCommand =>
                command instanceof ChatConversationUpsertCommand && command.entity.status === 'error'
        )

        expect(executionErrorCommand?.execution).toMatchObject({
            id: 'execution-1',
            status: XpertAgentExecutionStatusEnum.ERROR,
            error: 'No strategy found for type docker-sandbox'
        })
        expect(messageErrorCommand?.entity).toMatchObject({
            id: 'ai-1',
            status: XpertAgentExecutionStatusEnum.ERROR,
            error: 'No strategy found for type docker-sandbox'
        })
        expect(conversationErrorCommand?.entity).toMatchObject({
            id: 'conversation-1',
            status: 'error',
            error: 'No strategy found for type docker-sandbox'
        })
