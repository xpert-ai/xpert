...
describe('SandboxShellMiddleware', () => {
    ...
    it('registers only the shell tool', async () => {
        ...
        const schema = agentMiddleware.tools[0].schema
        if (!('shape' in schema)) throw new Error('Expected object schema')
        expect(Object.keys(schema.shape).sort()).toEqual(['command', 'timeout_sec'])
    })
