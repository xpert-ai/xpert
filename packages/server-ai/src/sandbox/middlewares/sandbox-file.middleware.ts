...
const indentationSchema = z
    .object({
        anchor_line: z
            .number()
            .optional()
            .describe('Anchor line to center the indentation lookup on (defaults to offset)'),
        max_levels: z.number().optional().describe('How many parent indentation levels to include (0 = unlimited)'),
        include_siblings: z
            .boolean()
            .optional()
            .describe('When true, include additional blocks that share the anchor indentation'),
        include_header: z
            .boolean()
            .optional()
            .describe('Include doc comments or attributes directly above the selected block'),
...
const readToolSchema = z.object({
    ...
    mode: z
        ...
        .optional()
        .describe('Mode: "slice" for simple ranges (default), "indentation" to expand around anchor line'),
...
const globToolSchema = z.object({
    pattern: z
        ...
        .describe('The glob pattern to match files against (e.g., "**/*.ts", "src/*.py")'),
    path: z
        .string()
        .optional()
        .describe(
    ...
    edits: z
        .array(
            z.object({
                oldString: z
                    .string()
                    .min(1, 'Old string is required.')
                    .describe('Exact text to replace (must match file content)'),
...
const writeToolSchema = z.object({
    ...
    content: z
        .any()
        .describe(
            'Complete file content as a single string. Special characters (quotes, newlines, etc.) must be properly escaped in JSON.'
        )
})
...
const appendToolSchema = z.object({
    ...
    content: z
        .any()
        .describe(
            'Content to append to the file. Special characters (quotes, newlines, etc.) must be properly escaped in JSON.'
        )
})
    ...
    createMiddleware(_options: unknown, context: IAgentMiddlewareContext): PromiseOrValue<AgentMiddleware> {
        ...
        const readTool = tool(
            ...
            },
            {
                ...
                description:
                    'Reads a file in the sandbox with 1-indexed line numbers, supporting slice and indentation-aware block modes.',
        ...
        const globTool = tool(
            ...
            },
            {
                name: 'sandbox_glob',
                description:
                    'Fast file pattern matching tool. Returns matching file paths sorted by modification time. Use this when you need to find files by name patterns.',
        ...
        const grepTool = tool(
            ...
            },
            {
                name: 'sandbox_grep',
                description:
                    'Fast content search tool using regex. Returns matching file paths and line numbers, sorted by modification time. Use include parameter to filter files by pattern.',
        ...
        const editTool = tool(
            ...
            },
            {
                ...
                description:
                    'Edit a file by replacing exact text. Returns edit metadata and errors when matches are missing or ambiguous.',
...
3. All special characters (quotes, newlines, etc.) must be properly JSON-escaped`,
        ...
        const multiEditTool = tool(
            ...
            },
            {
                ...
                description:
                    'Perform multiple sequential edits on a single file in one operation. All edits are applied sequentially, with each edit operating on the result of the previous edit. All edits must succeed for the operation to succeed (atomic). Use this when you need to make multiple changes to the same file.',
        ...
        const listDirTool = tool(
            ...
            },
            {
                ...
                description:
                    'List directory contents recursively with depth control and pagination. Returns a tree-like view with indentation showing directory structure. Directories end with /, symlinks with @. Use offset and limit for pagination through large directories.',
