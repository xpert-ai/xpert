    ...
    implements SandboxManagedServiceAdapter, SandboxServiceProxyAdapter, SandboxTerminalAdapter
{
                ...
                (upstreamResponse) => {
                    ...
                    upstreamResponse.on('end', () => {
                        request.response.end(
                            rewritePreviewTextResponse(Buffer.concat(chunks).toString('utf8'), proxyBasePath)
                        )
                        resolve()
                    })
                }
    ...
    private runCommand(
        ...
        return new Promise((resolve) => {
            const resolvedOptions = resolveSandboxExecutionOptions(
                executionOptions,
                DEFAULT_SANDBOX_SHELL_EXECUTION_OPTIONS
            )
            const chunks: string[] = []
    ...
    private resolveSandboxFilePath(filePath: string): string {
        ...
        if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) {
            const error = new Error(
                `Path is outside of the sandbox working directory: ${filePath}`
