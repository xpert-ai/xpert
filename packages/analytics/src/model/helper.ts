...
import { DataSourceStrategyQuery } from '@xpert-ai/server-core'
 ...
 * @param model
 */
export async function updateXmlaCatalogContent(
	queryBus: QueryBus,
	...
	if (
		...
		model.options.schema.cubes?.length
	) {
		...
		const query_runner = await queryBus.execute(
			new DataSourceStrategyQuery(
				model.dataSource.type.type,
				<AdapterBaseOptions>(<unknown>(model.dataSource.options ?? {}))
			)
		)
		const name = model.id
